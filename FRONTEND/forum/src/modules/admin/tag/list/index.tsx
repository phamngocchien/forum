...
import { deleteTag, getTagById } from "store/tag/action";
...
export default function TagList({ data }: TagProps) {
  ...
  const handleClickOpenTag = (id: string) => {
  ...
  return (
    <>
          ...
          <TableBody>
            {(rowsPerPage > 0
              ...
              <TableRow key={row.name}>
                ...
                <TableCell>
                  <Box className="truncate-config">{row.description}</Box>
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <ButtonGroup disableElevation>
                    <Button onClick={() => handleClickOpenTag(row._id)}>
                      <EditIcon />
                    </Button>
      ...
      >
        <DialogTitle>{"Do you want to delete this tag?"}</DialogTitle>
        <DialogActions>
          <Button onClick={handleConfirmDelete}>Agree</Button>
          <Button onClick={handleCloseConfirm}>Cancel</Button>
        </DialogActions>
      </Dialog>
      <FormTag mode={"edit"} isOpen={openTag} onClose={handleCloseTag} />
    </>
  );
}
